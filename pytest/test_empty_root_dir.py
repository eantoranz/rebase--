...
def test_empty_root_dir(tmp_path):
    ...
    main_commits.append(create_commit(repo, main_tree, "Adding C.txt", main_commits[-1:]))
    ...
    main_commits.append(create_commit(repo, main_tree, "Removing C.txt", main_commits[-1:]))
    the_tree = repo.get(main_commits[-1]).tree
    assert len(the_tree) == 1
    the_directory = the_tree["a-dir"]
    assert isinstance(the_directory, pygit2.Tree)
    ...
    test_commit = create_commit(repo, merge.write_tree(), "Merging A into test", [B_commits[-1], A_commits[-1]])
